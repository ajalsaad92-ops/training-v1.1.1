...
import {
  ...
  Mail, ClipboardList, AlertCircle, Users as UsersIcon, FolderArchive,
...
const ManagerDashboard = () => {
  ...
  const corrInProgress = archiveDocs.filter(d => d.forCheck === "1").length;
  ...
  const onLeave = monthApproved.filter(r => r.type.includes("?????"));
  const onTimeOff = monthApproved.filter(r => r.type === "??????");
  const onDuty = monthApproved.filter(r => r.type === "????");
  const absentToday = hrRequests.filter(r => r.type === "????" && r.date === todayStr && r.approval_status === "approved");
  ...
  return (
    <div className="space-y-4">
      ...
      <div data-print-section="attendance" className="bg-card border border-border rounded-xl p-4">
        <SectionHeader icon={UsersIcon} title="?????? ??????" action={<Button variant="link" size="sm" className="h-auto p-0 text-[10px]" onClick={() => navigate("/hr")}>??? ????</Button>} />
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
          ...
          <div className="bg-success/5 border border-success/20 rounded-lg p-3 cursor-pointer hover:shadow-sm transition-all" onClick={() => navigate("/hr")}>
            ...
            <p className="text-[10px] text-muted-foreground">????: {dailyCount} | ?????: {shiftCount}</p>
          </div>
          ...
          <div className="bg-warning/5 border border-warning/20 rounded-lg p-3">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-xs font-bold text-warning">???????? ({onLeave.length})</span>
            </div>
            <div className="space-y-1 flex-1 overflow-y-auto">
              {onLeave.length === 0 ? <p className="text-[10px] text-muted-foreground">?? ???? ??????</p> :
                Object.entries(leaveUsage).map(([name, days]) => (
                  <PersonRow key={name} name={name} detail={`${days}/${MONTHLY_LEAVE} ???`} status="leave" onClick={() => navigateToPerson(name)} />
                ))
              }
            </div>
          </div>
          ...
          <div className="bg-primary/5 border border-primary/20 rounded-lg p-3">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-xs font-bold text-primary">???????? ({onTimeOff.length})</span>
            </div>
            <div className="space-y-1 flex-1 overflow-y-auto">
              {onTimeOff.length === 0 ? <p className="text-[10px] text-muted-foreground">?? ???? ??????</p> :
                Object.entries(timeUsage).map(([name, hrs]) => (
                  <PersonRow key={name} name={name} detail={`${hrs}/${MONTHLY_TIME_HOURS} ????`} status="timeoff" onClick={() => navigateToPerson(name)} />
                ))
              }
            </div>
          </div>
          ...
          <div className="space-y-2">
            <div className="bg-destructive/5 border border-destructive/20 rounded-lg p-2 cursor-pointer hover:shadow-sm transition-all" onClick={() => navigate("/hr")}>
              <div className="flex items-center gap-1.5"><UserX className="w-3.5 h-3.5 text-destructive" /><span className="text-[11px] font-bold text-destructive">???????? ({absentToday.length})</span></div>
            ...
            <div className="bg-accent/5 border border-accent/20 rounded-lg p-2 cursor-pointer hover:shadow-sm transition-all" onClick={() => navigate("/hr")}>
              <div className="flex items-center gap-1.5"><Briefcase className="w-3.5 h-3.5 text-accent" /><span className="text-[11px] font-bold text-accent">???????? ({onDuty.length})</span></div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        <div data-print-section="pending_requests" className="bg-card border border-border rounded-xl p-4">
          <SectionHeader icon={ClipboardList} title={`????? ????? (${pendingHR.length})`} action={<Button variant="link" size="sm" className="h-auto p-0 text-[10px]" onClick={() => navigate("/hr")}>??? ????</Button>} />
          {pendingHR.length === 0 ? <p className="text-xs text-muted-foreground text-center py-4">?? ???? ?????</p> : (
                ...
                <div key={req.id} className="flex items-center justify-between bg-muted/20 rounded-lg px-3 py-2">
                  <div className="min-w-0 flex-1"><p className="text-xs font-medium truncate">{req.employee_name}</p><p className="text-[10px] text-muted-foreground">{req.type} — {req.date}</p></div>
                   <div className="flex gap-1 shrink-0 no-print">
                     {has("approve_hr_dept") && <button onClick={() => handleHRAction(req.id, "approve")} className="p-1 rounded bg-success/10 text-success hover:bg-success/20" title="??????"><Check className="w-3 h-3" /></button>}
                     {has("reject_hr") && <button onClick={() => handleHRAction(req.id, "reject")} className="p-1 rounded bg-destructive/10 text-destructive hover:bg-destructive/20" title="???"><X className="w-3 h-3" /></button>}
                   </div>
                </div>
              ))}
            </div>
        ...
        <div data-print-section="tasks" className="bg-card border border-border rounded-xl p-4">
          <SectionHeader icon={ListChecks} title={`?????? (${tasks.length})`} action={<Button variant="link" size="sm" className="h-auto p-0 text-[10px]" onClick={() => navigate("/tasks")}>??? ????</Button>} />
          <div className="space-y-1.5 flex-1 overflow-y-auto">
            {tasks.slice(0, 6).map(t => (
              <div key={t.id} className="flex items-center justify-between bg-muted/20 rounded-lg px-3 py-2 cursor-pointer hover:bg-muted/30 transition-colors" onClick={() => navigate("/tasks")}>
                ...
                <StatusBadge status={t.status === "completed" ? "????" : t.status === "in_progress" ? "??? ???????" : t.status === "review" ? "??????" : "????"} />
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
        <div data-print-section="alerts" className="bg-card border border-border rounded-xl p-4 space-y-2">
          <p className="text-xs font-bold text-foreground flex items-center gap-1.5"><AlertCircle className="w-3.5 h-3.5 text-warning" />???????</p>
          {missingReports > 0 && <div className="flex items-center gap-2 bg-destructive/10 rounded-lg px-3 py-2 cursor-pointer hover:shadow-sm transition-all" onClick={() => navigate("/curriculum?filter=missing_report")}><BookX className="w-4 h-4 text-destructive shrink-0" /><span className="text-xs">{missingReports} ???? ???? ?????</span></div>}
          {missingPPT > 0 && <div className="flex items-center gap-2 bg-warning/10 rounded-lg px-3 py-2 cursor-pointer hover:shadow-sm transition-all" onClick={() => navigate("/curriculum?filter=missing_ppt")}><Presentation className="w-4 h-4 text-warning shrink-0" /><span className="text-xs">{missingPPT} ???? ???? ???</span></div>}
          {pendingHR.length > 0 && <div className="flex items-center gap-2 bg-primary/10 rounded-lg px-3 py-2 cursor-pointer hover:shadow-sm transition-all" onClick={() => navigate("/hr?filter=pending")}><ClipboardList className="w-4 h-4 text-primary shrink-0" /><span className="text-xs">{pendingHR.length} ??? ????? ????</span></div>}
          {corrInProgress > 0 && <div className="flex items-center gap-2 bg-accent/10 rounded-lg px-3 py-2 cursor-pointer hover:shadow-sm transition-all" onClick={() => navigate("/archive")}><FolderArchive className="w-4 h-4 text-accent shrink-0" /><span className="text-xs">{corrInProgress} ????? ??? ????????</span></div>}
          {missingReports === 0 && missingPPT === 0 && pendingHR.length === 0 && corrInProgress === 0 && (
            <div className="flex items-center gap-2 bg-success/10 rounded-lg px-3 py-2"><Check className="w-4 h-4 text-success shrink-0" /><span className="text-xs">?? ???? ???????</span></div>
          )}
          <div className="border-t border-border pt-2 mt-1">
            <p className="text-[10px] font-bold text-muted-foreground mb-1">??? ?????????</p>
        ...
        <div data-print-section="achievement" className="bg-card border border-border rounded-xl p-4 flex flex-col items-center gap-3 cursor-pointer hover:shadow-sm transition-all" onClick={() => navigate("/courses")}>
          ...
          <div className="w-full space-y-1.5">
            <MiniBar value={activeCourses.length} max={courses.length || 1} color="bg-primary" label="????" />
            <MiniBar value={plannedCourses.length} max={courses.length || 1} color="bg-warning" label="?????" />
            <MiniBar value={completedCourses.length} max={courses.length || 1} color="bg-success" label="?????" />
          </div>
        </div>
      </div>
...
const PrepDashboard = () => {
  ...
  const handleUnitApprove = (id: string) => { localDb.hrRequests.update(id, { approval_status: "unit_approved", unit_head_status: "approved" }); toast({ title: "?????? ???? ??????" }); refetchHR(); };
  const handleReject = (id: string) => { localDb.hrRequests.update(id, { approval_status: "rejected", unit_head_status: "rejected" }); toast({ title: "?? ?????" }); refetchHR(); };
    ...
    <div className="space-y-4">
      ...
      <div data-print-section="attendance" className="bg-card border border-border rounded-xl p-4">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          <div className="bg-success/5 border border-success/20 rounded-lg p-3 cursor-pointer hover:shadow-sm transition-all" onClick={() => navigate("/hr")}>
            <div className="flex items-center gap-2 mb-1"><UserCheck className="w-4 h-4 text-success" /><span className="text-xs font-bold text-success">????????</span></div>
            <p className="text-2xl font-bold">{employees.length - onLeave.length - absentToday.length}</p>
          </div>
          <div className="bg-warning/5 border border-warning/20 rounded-lg p-3">
            <div className="flex items-center gap-2 mb-1"><CalendarPlus className="w-4 h-4 text-warning" /><span className="text-xs font-bold text-warning">???????? ({onLeave.length})</span></div>
          ...
          <div className="bg-primary/5 border border-primary/20 rounded-lg p-3">
            <div className="flex items-center gap-2 mb-1"><Timer className="w-4 h-4 text-primary" /><span className="text-xs font-bold text-primary">???????? ({onTimeOff.length})</span></div>
            <div className="space-y-0.5 flex-1 overflow-y-auto">
              {onTimeOff.length === 0 ? <p className="text-[10px] text-muted-foreground">?? ????</p> :
                Object.entries(timeUsage).map(([name, hrs]) => <PersonRow key={name} name={name} detail={`${hrs}/${MONTHLY_TIME_HOURS}?`} status="timeoff" onClick={() => navigate("/hr")} />)}
            </div>
          </div>
          <div className="bg-destructive/5 border border-destructive/20 rounded-lg p-3 cursor-pointer hover:shadow-sm transition-all" onClick={() => navigate("/hr")}>
            <div className="flex items-center gap-2 mb-1"><UserX className="w-4 h-4 text-destructive" /><span className="text-xs font-bold text-destructive">???????? ({absentToday.length})</span></div>
            {absentToday.slice(0, 2).map(r => <p key={r.id} className="text-[10px]">{r.employee_name}</p>)}
            {absentToday.length === 0 && <p className="text-[10px] text-muted-foreground">?? ????</p>}
          </div>
        </div>
      </div>
      ...
      {/* ROW 3: ?????? */}
      <div data-print-section="tasks" className="bg-card border border-border rounded-xl p-4">
        <SectionHeader icon={ListChecks} title={`????? (${myTasks.length})`} action={<Button variant="link" size="sm" className="h-auto p-0 text-[10px]" onClick={() => navigate("/tasks")}>??? ????</Button>} />
        <div className="space-y-1.5 flex-1 overflow-y-auto">
          {myTasks.slice(0, 5).map(t => (
            <div key={t.id} className="flex items-center justify-between bg-muted/20 rounded-lg px-3 py-2 cursor-pointer hover:bg-muted/30" onClick={() => navigate("/tasks")}>
              ...
              <StatusBadge status={t.status === "in_progress" ? "??? ???????" : t.status === "review" ? "??????" : "????"} />
            </div>
          ))}
        </div>
      </div>
  ...
  return (
    <div className="space-y-4">
      <div data-print-section="curriculum_stages" className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <div className="bg-card border border-border rounded-xl p-3 hover:shadow-md transition-all cursor-pointer" onClick={() => navigate("/curriculum")}><div className="flex items-center gap-2 mb-1"><FileText className="w-4 h-4 text-primary" /><span className="text-xs font-bold">?????? ???????</span></div><p className="text-2xl font-bold">{totalCur}</p></div>
        <div className="bg-card border border-border rounded-xl p-3 hover:shadow-md transition-all cursor-pointer" onClick={() => navigate("/curriculum")}><div className="flex items-center gap-2 mb-1"><BookX className="w-4 h-4 text-destructive" /><span className="text-xs font-bold">?????? ??????</span></div><p className="text-2xl font-bold text-destructive">{missingReports}</p></div>
        <div className="bg-card border border-border rounded-xl p-3 hover:shadow-md transition-all cursor-pointer" onClick={() => navigate("/curriculum")}><div className="flex items-center gap-2 mb-1"><Presentation className="w-4 h-4 text-warning" /><span className="text-xs font-bold">???? ??????</span></div><p className="text-2xl font-bold text-warning">{missingPPT}</p></div>
        <div className="bg-card border border-border rounded-xl p-3 hover:shadow-md transition-all cursor-pointer" onClick={() => navigate("/curriculum")}><div className="flex items-center gap-2 mb-1"><AlertCircle className="w-4 h-4 text-destructive" /><span className="text-xs font-bold">??? ??????</span></div><p className="text-2xl font-bold text-destructive">{incompleteFields.length}</p></div>
      </div>
      ...
      <div data-print-section="achievement" className="grid grid-cols-1 md:grid-cols-2 gap-3">
        <div className="bg-card border border-border rounded-xl p-4 cursor-pointer hover:shadow-sm transition-all" onClick={() => navigate("/curriculum")}>
          <p className="text-xs font-bold text-foreground mb-3">????? ???????</p>
      ...
      <div data-print-section="alerts" className="bg-card border border-border rounded-xl p-4">
        <SectionHeader icon={ListChecks} title={`?????? (${curTasks.length})`} action={<Button variant="link" size="sm" className="h-auto p-0 text-[10px]" onClick={() => navigate("/tasks")}>??? ????</Button>} />
        <div className="space-y-1.5 flex-1 overflow-y-auto">
...
const IndividualDashboard = () => {
  ...
  const usedLeaves = myApprovedHR.filter(r => r.type.includes("?????")).length;
  const remainingLeaves = Math.max(MONTHLY_LEAVE - usedLeaves, 0);
  const usedTime = myApprovedHR.filter(r => r.type === "??????").length;

  return (
    <div className="space-y-4">
      {/* ROW 1: ?????? ?????? */}
      <div data-print-section="attendance" className="bg-card border border-border rounded-xl p-4">
        <SectionHeader icon={UsersIcon} title="?????" />
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          <div className={`border rounded-lg p-3 cursor-pointer hover:shadow-sm transition-all ${remainingLeaves === 0 ? "bg-destructive/5 border-destructive/20" : "bg-success/5 border-success/20"}`} onClick={() => navigate("/hr")}>
            <div className="flex items-center gap-2 mb-1"><CalendarPlus className="w-4 h-4 text-warning" /><span className="text-xs font-bold text-warning">???? ????????</span></div>
            <p className="text-2xl font-bold">{remainingLeaves}/{MONTHLY_LEAVE}</p>
            <p className="text-[10px] text-muted-foreground">??????: {usedLeaves} ???</p>
          </div>
          <div className="bg-primary/5 border border-primary/20 rounded-lg p-3 cursor-pointer hover:shadow-sm transition-all" onClick={() => navigate("/hr")}>
            <div className="flex items-center gap-2 mb-1"><Timer className="w-4 h-4 text-primary" /><span className="text-xs font-bold text-primary">????????</span></div>
            <p className="text-2xl font-bold">{MONTHLY_TIME_HOURS - usedTime}/{MONTHLY_TIME_HOURS}</p>
            <p className="text-[10px] text-muted-foreground">??????: {usedTime} ????</p>
          </div>
          <div className="bg-accent/5 border border-accent/20 rounded-lg p-3 cursor-pointer hover:shadow-sm transition-all" onClick={() => navigate("/courses")}>
            <div className="flex items-center gap-2 mb-1"><BookOpen className="w-4 h-4 text-accent" /><span className="text-xs font-bold text-accent">??????</span></div>
          ...
          <div className="bg-primary/5 border border-primary/20 rounded-lg p-3 cursor-pointer hover:shadow-sm transition-all" onClick={() => navigate("/tasks")}>
            <div className="flex items-center gap-2 mb-1"><ListChecks className="w-4 h-4 text-primary" /><span className="text-xs font-bold text-primary">????? ???????</span></div>
            <p className="text-2xl font-bold">{myPendingTasks.length}</p>
            <p className="text-[10px] text-muted-foreground">{myCompletedTasks.length} ?????</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        <div data-print-section="pending_requests" className="bg-card border border-border rounded-xl p-4">
          <SectionHeader icon={ClipboardList} title={`?????? (${myHR.length})`} action={<Button variant="link" size="sm" className="h-auto p-0 text-[10px]" onClick={() => navigate("/hr")}>??? ????</Button>} />
          {myHR.length === 0 ? <p className="text-xs text-muted-foreground text-center py-4">?? ???? ?????</p> : (
            <div className="space-y-1.5 flex-1 overflow-y-auto">
              {myHR.map(r => (
                <div key={r.id} className="flex items-center justify-between bg-muted/20 rounded-lg px-3 py-2 cursor-pointer hover:bg-muted/30" onClick={() => navigate("/hr")}>
                  ...
                  <StatusBadge status={r.approval_status === "approved" ? "?????" : r.approval_status === "rejected" ? "?????" : r.approval_status === "unit_approved" ? "?????? ????" : "????"} />
                </div>
              ))}
            </div>
          )}
        ...
        <div data-print-section="tasks" className="bg-card border border-border rounded-xl p-4">
          <SectionHeader icon={ListChecks} title={`????? (${myTasks.length})`} action={<Button variant="link" size="sm" className="h-auto p-0 text-[10px]" onClick={() => navigate("/tasks")}>??? ????</Button>} />
          {myTasks.length === 0 ? <p className="text-xs text-muted-foreground text-center py-4">?? ???? ????</p> : (
            <div className="space-y-1.5 flex-1 overflow-y-auto">
              {myTasks.map(t => (
                <div key={t.id} className="flex items-center justify-between bg-muted/20 rounded-lg px-3 py-2 cursor-pointer hover:bg-muted/30" onClick={() => navigate("/tasks")}>
                  ...
                  <StatusBadge status={t.status === "completed" ? "????" : t.status === "in_progress" ? "??? ???????" : "????"} />
                </div>
              ))}
            </div>
          )}
      ...
      <div data-print-section="curriculum_quick" className="bg-card border border-border rounded-xl p-4">
        <SectionHeader icon={BookOpen} title="?????? ?????????" action={<Button variant="link" size="sm" className="h-auto p-0 text-[10px]" onClick={() => navigate("/courses")}>??? ????</Button>} />
        {myTrainings.length === 0 ? <p className="text-xs text-muted-foreground text-center py-4">?? ???? ?????</p> : (
          <div className="space-y-1.5">
            {myTrainings.map(c => (
              <div key={c.id} className="flex items-center justify-between bg-muted/20 rounded-lg px-3 py-2 cursor-pointer hover:bg-muted/30" onClick={() => navigate("/courses")}>
                ...
                <StatusBadge status={c.status === "active" ? "????" : c.status === "completed" ? "?????" : "?????"} />
              </div>
            ))}
          </div>
        )}
...
const Dashboard = () => {
  ...
  if (!user) return <div className="flex flex-col items-center justify-center py-20 gap-3"><AlertCircle className="w-12 h-12 text-muted-foreground" /><p className="text-sm text-muted-foreground">???? ????? ??????</p></div>;

  const getDashboardSections = (): { id: string; label: string }[] => {
    const sections: { id: string; label: string }[] = [];
    if (has("view_attendance_section")) sections.push({ id: "attendance", label: "?????? ??????" });
    if (has("view_pending_requests_section")) sections.push({ id: "pending_requests", label: "??????? ???????" });
    if (has("view_tasks_section")) sections.push({ id: "tasks", label: "??????" });
    if (has("view_alerts_section")) sections.push({ id: "alerts", label: "?????????" });
    if (has("view_curriculum_stages_section")) sections.push({ id: "curriculum_stages", label: "????? ???????" });
    if (has("view_achievement_section")) sections.push({ id: "achievement", label: "???? ???????" });
    if (has("view_curriculum_quick_section")) sections.push({ id: "curriculum_quick", label: "??????? ???? ?????" });
    return sections;
  };
  ...
  return (
    <div className="h-full flex flex-col p-2 md:p-3 overflow-hidden">
      <PageHeader title="???? ??????" subtitle={`?????? ${user.name} — ${user.position}`} icon={LayoutDashboard} sections={dashboardSections} />
