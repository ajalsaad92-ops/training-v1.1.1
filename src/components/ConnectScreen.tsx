...
const ConnectScreen = () => {
  ...
  const [testing, setTesting] = useState(false);import { useState, useEffect } from "react";
import { Wifi, WifiOff, Loader2, Monitor, Smartphone, RefreshCw, Search } from "lucide-react";
import { discoverServer } from "@/lib/sync/localServerSync";
...
const ConnectScreen = () => {
  ...
  useEffect(() => {
    // FIX #3: Read saved IP from appConfig (single source of truth for sync layer)
    const cfg = getConfig();
    if (cfg.localServer?.host && cfg.localServer.host !== "127.0.0.1") {
      setIp(cfg.localServer.host);
    } else {
  ...
  const connectTo = async (trimmed: string) => {
    ...
    const cfg = getConfig();
    const port = cfg.localServer?.port || 3003;
    const base = trimmed.startsWith("http") ? trimmed : `http://${trimmed}:${port}`;

    try {
      const res = await fetch(base + "/api/ping", { signal: AbortSignal.timeout(5000) });
      const data = await res.json();
      if (data.ok) {
        // Extract clean host (no protocol, no port)
        const hostOnly = trimmed
          .replace(/^https?:\/\//, "")
          .replace(/:\d+$/, "")
          .trim();

        // FIX #3: Save to appConfig so the sync layer reads the correct host
        setConfig({
          mode: "local",
          serverRole: "client",
          localServer: { host: hostOnly, port },
        });

        // Keep legacy key for backward compatibility
        localStorage.setItem("tms_laptop_ip", hostOnly);

        setSuccess(true);

        // FIX #2: NO window.location.href redirect — that kills the Capacitor bridge!
        // setConfig() dispatches tms_config_changed → App.tsx re-pings the server
        // → serverOk becomes true → ConnectScreen unmounts automatically.
        reinitSync();
      } else {
        setError("الخادم استجاب لكن بشكل غير صحيح");
      }
    } catch {
      setError(
        "تعذر الاتصال بالخادم. تأكد من أن الحاسوب والهاتف على نفس شبكة WiFi وأن الخادم يعمل"
      );
    }
    setTesting(false);
  };
        ...
        <div className="bg-card border rounded-2xl p-6 space-y-5 text-right shadow-sm">
          ...
          <div className="flex items-center justify-center">
            <div className="flex flex-col items-center gap-1">
              ...
              <Wifi className={`w-6 h-6 ${success ? "text-green-500" : "text-muted-foreground"}`} />
              <div className="w-px h-8 bg-border"></div>
            </div>
          </div>
          ...
          {success && (
            <div className="flex items-center gap-2 bg-green-500/10 text-green-600 rounded-xl p-3">
              ...
              <span className="text-xs font-medium">تم الاتصال بنجاح! جارٍ تهيئة المزامنة...</span>
            </div>
          )}
